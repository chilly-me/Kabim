...
// Maps bedroom count to folder name
const BED_TO_FOLDER = { 1: 'one', 2: 'two', 3: 'three' };

// How many variants currently exist per bedroom count.
// Update this as you add folders 2-10 under each subfolder.
const VARIANT_COUNTS = { 1: 1, 2: 1, 3: 1 };

/**
 * Picks a random variant number for the given bedroom count,
 * then returns the paths to the model and floor plan.
 * e.g. bedrooms=3 → /houses/three/1/model.glb
 */
function resolveHousePaths(bedrooms) {
  const folder  = BED_TO_FOLDER[bedrooms] ?? 'two';
  const count   = VARIANT_COUNTS[bedrooms] ?? 1;
  const variant = Math.floor(Math.random() * count) + 1; // 1-indexed
  const base    = `/houses/${folder}/${variant}`;
  return {
    glbPath:      `${base}/model.glb`,
    planPath:     `${base}/houseplan.png`,
    folder,
    variant,
  };
}
...
const ACCENT = '#c9a96e';
const dk = {
  bg: '#1a1a1a', surface: '#1e1e1e', surface2: '#222',
...
const SectionLabel = ({ children }) => (
  <div style={{ fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: dk.dim, marginBottom: 14 }}>{children}</div>
);
...
const panelStyle = { padding: '20px 16px', overflowY: 'auto', height: 'calc(100vh - 104px)', boxSizing: 'border-box' };

// ─── PANELS ───────────────────────────────────────────────────────────────────

const HousePlanPanel = ({ planPath, bedrooms }) => (
  <div style={panelStyle}>
    <SectionLabel>Floor plan — ground level</SectionLabel>

    {/* Floor plan image */}
    <div style={{ width: '100%', borderRadius: 12, overflow: 'hidden', border: `1px solid ${dk.border}`, background: dk.surface, minHeight: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <img
        src={planPath}
        alt={`${bedrooms}-bedroom bungalow floor plan`}
      ...
      />
      <div style={{ display: 'none', flexDirection: 'column', alignItems: 'center', gap: 8, padding: 40, color: dk.muted }}>
        <span style={{ fontSize: 36 }}>📐</span>
        <span style={{ fontSize: 13 }}>Floor plan not found</span>
        <span style={{ fontSize: 11, color: dk.dim }}>{planPath}</span>
      </div>
    </div>
    ...
    <div style={{ marginTop: 24 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1, borderRadius: 8, overflow: 'hidden', border: `1px solid ${dk.border}` }}>
        {[
          ...
          ...(bedrooms >= 2 ? [{ room: 'Bedroom 2', area: '12.0 m²', dims: '3.0 × 4.0 m' }] : []),
          ...(bedrooms >= 3 ? [{ room: 'Bedroom 3', area: '10.5 m²', dims: '3.0 × 3.5 m' }] : []),
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', fontSize: 13, background: '#2a2a2a', borderTop: `1px solid ${dk.border}` }}>
          ...
          <span style={{ color: ACCENT, fontWeight: 500 }}>{bedrooms === 1 ? '65.0' : bedrooms === 2 ? '98.0' : '130.0'} m²</span>
        </div>
      </div>
    </div>
  </div>
...
const Cottage = () => {
  ...
  // Read bedrooms from navigation state; default to 2 if accessed directly
  const bedrooms = location.state?.bedrooms ?? 2;

  // Resolve paths once on mount — useMemo so the random pick doesn't
  // re-roll every time the component re-renders (e.g. tab switch)
  const { glbPath, planPath } = useMemo(() => resolveHousePaths(bedrooms), [bedrooms]);
    ...
    <div style={{ width: '100%', height: '100vh', display: 'flex', flexDirection: 'column', background: dk.bg, color: dk.text, fontFamily: 'helvetica neue, helvetica, arial, sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 0', borderBottom: `1px solid ${dk.border}`, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 14 }}>
          <button onClick={() => navigate('/')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: dk.muted, display: 'flex', alignItems: 'center', padding: 0 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>
          </button>
          <div style={{ width: 1, height: 16, background: dk.border }} />
          <div>
            <div style={{ fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', color: dk.dim }}>Bungalow</div>
            <div style={{ fontSize: 14, fontWeight: 500, color: dk.text }}>{bedroomLabel}</div>
          </div>
        </div>
      ...
      {/* Panel — pass resolved paths down to the panels that need them */}
      <div style={{ flex: 1, overflow: 'hidden' }}>
        {activeTab === 'plan'     && <HousePlanPanel planPath={planPath} bedrooms={bedrooms} />}
        {activeTab === 'model'    && <ModelPanel glbPath={glbPath} />}
